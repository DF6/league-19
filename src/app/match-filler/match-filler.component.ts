...
export class MatchFillerComponent implements OnInit{
    ...
    private resolveKO(match) {
        ...
        if(almostFilledMatches.length > 0) {
            ...
            this.http.post('./CMDataRequesting.php', {type: 'ediMat', id: chosenMatch.id, local: chosenMatch.local, away: this.appService.whoWon(match), tournament: chosenMatch.tournament, round: chosenMatch.round + 1}).subscribe( (response) => {
            ...
            this.http.post('./CMDataRequesting.php', {type: 'insMat', local: this.appService.whoWon(match), away: 0, tournament: match.tournament, round: match.round + 1}).subscribe( (response) => {
                if (response.json().success) {
