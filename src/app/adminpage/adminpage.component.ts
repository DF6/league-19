...
export class AdminPageComponent implements OnInit{
    ...
    public recalculateStandings() {
        this.http.post('./test_CMDataRequesting.php', {type: 'resSta', tournament: this.tournamentToReset.id}).subscribe( (response) => {
            if(response.json().success) {
                this.matches.forEach( (value) => {
                    if(value.tournament == this.tournamentToReset.id && value.localGoals != -1) {
                        ...
                        this.http.post('./test_CMDataRequesting.php', {type: 'updSta', points: local.points, won: local.won, draw: local.draw, lost: local.lost, goalsFor: parseInt(value.localGoals), goalsAgainst: parseInt(value.awayGoals), tournamentID: value.tournament, team: value.local}).subscribe( () => {});
                        this.http.post('./test_CMDataRequesting.php', {type: 'updSta', points: away.points, won: away.won, draw: away.draw, lost: away.lost, goalsFor: parseInt(value.awayGoals), goalsAgainst: parseInt(value.localGoals), tournamentID: value.tournament, team: value.away}).subscribe( () => {});
                    }
                });
